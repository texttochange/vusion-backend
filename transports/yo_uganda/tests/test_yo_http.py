...
from transports import YoHttpTransport

class YoUgHttpTransportTestCase(VumiTestCase):

    transport_type = 'sms'

    @inlineCallbacks
    def setUp(self):
        self.yo_calls = DeferredQueue()
        self.mock_yo = MockHttpServer(self.handle_request)
        self.mock_server_response = ''
        self.mock_server_response_code = http.OK
        yield self.mock_yo.start()
        
        self.config = {
            'url': self.mock_yo.url,
            'ybsacctno': 'ybsacctno',
            'password': 'password',
            'default_origin': '8282',
            'receive_path': '/yo',
            'receive_port': 9998}
        self.tx_helper = self.add_helper(TransportHelper(YoHttpTransport))
        self.transport = yield self.tx_helper.get_transport(self.config)

    @inlineCallbacks
    def tearDown(self):
        yield self.mock_yo.stop()
        yield super(YoUgHttpTransportTestCase, self).tearDown()

    def handle_request(self, request):
        self.yo_calls.put(request)
    ...
    def test_inbound(self):
        ...
        self.assertEqual('9292', user_msg['to_addr'])
